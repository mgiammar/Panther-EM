 ...
 * Input layout is the GEMM's native (NumFreq, P, Q) with NumFreq <= 64, as float2 or
 * __half2 (a float16 (F,P,2Q) tensor).
 */
#pragma once
#include <cfloat>
#include <cuda_fp16.h>
...
template <unsigned F, unsigned R, typename InT>
...
lean_irfft_stats_transposed(const InT* __restrict__ c, float* __restrict__ s1,
                            ...
                            unsigned P, unsigned Q, unsigned q_tiles) {
  static_assert(R == 2 || R == 4, "R in {2,4}");
  static_assert(F >= 1 && F <= 64, "NumFreq <= 64");
...
#pragma unroll
  ...
  if (h == 0) {
    ...
    pw = re[0] * re[0];
#pragma unroll
    for (unsigned k = 1; k < F; ++k) { pw = fmaf(2.f * re[k], re[k], pw); pw = fmaf(2.f * im[k], im[k], pw); }
  }
...
inline void launch_lean_transposed(const InT* c, float* s1, float* s2, unsigned long long* pk,
                                   unsigned P, unsigned Q, cudaStream_t stream) {
  constexpr unsigned PAIRS = 128 / (R / 2);
  const unsigned q_tiles = (Q + PAIRS - 1) / PAIRS;
  lean_irfft_stats_transposed<F, R, InT><<<P * q_tiles, 128, 0, stream>>>(c, s1, s2, pk, P, Q, q_tiles);
}
