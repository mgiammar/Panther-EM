...
static constexpr std::array<
    std::tuple<unsigned int, unsigned int, unsigned int, unsigned int>, 24>
    SUPPORTED_CONFIGS = {{
        // n_psi, num_freq, ept, fpb   (NOTE table order is (psi,freq,ept,fpb))
        {128, 64, 8, 8},  {128, 64, 16, 8}, {128, 64, 32, 8},
        {128, 64, 8, 16}, {128, 64, 16, 16}, {128, 64, 32, 16},
        {128, 64, 8, 4},  {128, 64, 16, 4}, {128, 64, 32, 4},
        {128, 64, 16, 32}, {128, 64, 32, 32}, {128, 64, 8, 2},
        {256, 64, 8, 8},  {256, 64, 16, 8}, {256, 64, 32, 8},
        {256, 64, 8, 16}, {256, 64, 16, 16}, {256, 64, 32, 16},
        {256, 64, 8, 4},  {256, 64, 16, 4}, {256, 64, 32, 4},
        {256, 64, 16, 32}, {256, 64, 32, 32}, {256, 64, 8, 2},
    }};
...
get_fn(unsigned int num_psi, unsigned int num_freq, unsigned int fpb, unsigned int ept) {
  for (const auto &e : dispatch_table) {
    if (e.first.num_psi == num_psi && e.first.num_freq == num_freq && e.first.fpb == fpb && e.first.ept == ept)
      return e.second;
  }
  return nullptr;
}
...
static std::function<void(const c10::complex<float> *, float *, float *,
                          unsigned long long *, unsigned int, unsigned int)>
get_fn_transposed(unsigned int num_psi, unsigned int num_freq, unsigned int fpb, unsigned int ept) {
 ...
 */
std::vector<torch::Tensor> fused_irfft_stats(torch::Tensor c, int64_t num_psi, int64_t fpb, int64_t ept) {
  ...
  auto fn = get_fn(static_cast<unsigned int>(num_psi), num_freq, static_cast<unsigned int>(fpb), static_cast<unsigned int>(ept));
...
std::vector<torch::Tensor> fused_irfft_stats_transposed(torch::Tensor c,
                                                         int64_t num_psi, int64_t fpb, int64_t ept) {
  TORCH_CHECK(c.is_cuda(), "c must be CUDA");
  TORCH_CHECK(c.dtype() == torch::kComplexFloat, "c must be complex64");
  ...
  auto fn = get_fn_transposed(static_cast<unsigned int>(num_psi), num_freq, static_cast<unsigned int>(fpb), static_cast<unsigned int>(ept));
  TORCH_CHECK(fn != nullptr, "Unsupported (num_psi, num_freq) = (", num_psi,
              ", ", num_freq, "). See get_supported_configs().");
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("fused_irfft_stats", &fused_irfft_stats,
        ...
        pybind11::arg("c"), pybind11::arg("num_psi"), pybind11::arg("fpb"), pybind11::arg("ept"));
