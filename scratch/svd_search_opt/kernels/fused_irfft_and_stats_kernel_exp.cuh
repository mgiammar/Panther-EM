...
// ---------------------------------------------------------------------------
// Input element conversion: the spectrum may be stored as complex64 (float2)
// or as fp16 pairs (__half2, i.e. a (.., 2Q) float16 tensor viewed pairwise).
// FFT arithmetic stays fp32 either way; only the global-memory footprint halves.
// ---------------------------------------------------------------------------
template <class C> __device__ __forceinline__ C to_cplx(float2 v) { return C{v.x, v.y}; }
template <class C> __device__ __forceinline__ C to_cplx(__half2 v) {
...
// ---------------------------------------------------------------------------
template <class FFT, unsigned int NumFreq, bool HasNyquist, typename InT>
...
#pragma unroll
  for (unsigned int i = 0; i < FFT::input_ept; ++i) {
    ...
    complex_type val = valid_read
                           ? to_cplx<complex_type>(input[batch_offset + read_idx])
...
__launch_bounds__(Config::FFT::max_threads_per_block) __global__
  ...
  load_padded_with_parseval_moments<FFT, num_freq, has_nyquist, InT>(
...
__launch_bounds__(Config::FFT::max_threads_per_block) __global__
    void fused_irfft_and_stats_kernel_transposed(
        const InT *__restrict__ c, // (NumFreq, P, Q)
        float *__restrict__ s1,                              // (P), pre-zeroed
        float *__restrict__ s2,                              // (P), pre-zeroed
        unsigned long long
            *__restrict__ argmax_packed, // (P), sentinel pre-filled
  ...
  // Cooperative, block-wide coalesced global->shared staging copy.
  {
    ...
    for (unsigned int elem = tid; elem < real_tile_elems;
      ...
      complex_type val = valid ? to_cplx<complex_type>(c[k * pq_stride + row_base + l])
...
template <class Config, typename InT>
inline void launch_fused_irfft_stats(const InT *c,
                                     float *s1, float *s2,
  ...
  if (!attr_set) {
    auto err =
        cudaFuncSetAttribute(fused_irfft_and_stats_kernel<Config, InT>,
  ...
  fused_irfft_and_stats_kernel<Config, InT>
...
inline void launch_fused_irfft_stats_transposed(
  ...
  if (!attr_set) {
    ...
    auto err = cudaFuncSetAttribute(
        fused_irfft_and_stats_kernel_transposed<Config, InT>,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
